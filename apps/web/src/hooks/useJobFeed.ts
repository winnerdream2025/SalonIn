...
export function useJobFeed(options: UseJobFeedOptions = {}): UseJobFeedResult {
  const { specialty, type, salonId, status } = options
  ...
  useEffect(() => {
    ...
    jobsApi
      .list({ lat, lng, radiusMiles, specialty, type, salonId, status, page: 1, limit: 20 })
      .then((res) => {
        if (!cancelled) {
          setJobs(res.data)
          setPage(1)
  ...
  }, [lat, lng, radiusMiles, specialty, type, salonId, status, tick])
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const res = await jobsApi.list({ lat, lng, radiusMiles, specialty, type, salonId, status, page: nextPage, limit: 20 })
  ...
  }, [hasMore, lat, lng, radiusMiles, specialty, type, salonId, status, page, isLoadingMore])
