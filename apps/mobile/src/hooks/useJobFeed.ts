...
export function useJobFeed(options: UseJobFeedOptions = {}): UseJobFeedResult {
  ...
  useEffect(() => {
    ...
    jobsApi
      .list({ cityId, specialty, type, listingType, page: 1, limit: 20 })
      .then((res) => {
        if (!cancelled) {
          setJobs(res.data)
          setPage(1)
  ...
  }, [cityId, specialty, type, listingType, tick])
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const res = await jobsApi.list({ cityId, specialty, type, listingType, page: nextPage, limit: 20 })
  ...
  }, [hasMore, cityId, specialty, type, listingType, page, isLoadingMore])
